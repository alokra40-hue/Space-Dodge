...
let gameState = "ready", lastTime = 0, spawnTimer = 0, powerupTimer = 5, elapsedTime = 0;
let score = 0, lives = 3, highScore = Number(localStorage.getItem("spaceDodgeHighScore") || 0);
let dodgeCooldown = 0, dodgeTime = 0, invulnerable = 0, shieldTime = 0, boostTime = 0, shakeTime = 0;
let soundOn = localStorage.getItem(SOUND_KEY) !== "off", audioCtx = null, enemies = [], powerups = [], particles = [], scoreRecorded = false;
...
function startGame() {
  showScreen("gameScreen"); player = createPlayer(); enemies = []; powerups = []; particles = [];
  spawnTimer = 0; powerupTimer = 5; elapsedTime = 0; score = 0; lives = 3; dodgeCooldown = 0; dodgeTime = 0; invulnerable = 0; shieldTime = 0; boostTime = 0; shakeTime = 0; scoreRecorded = false;
...
function update(dt) {
  ...
  updatePlayer(dt); spawnEnemies(dt); updateEnemies(dt); spawnPowerups(dt); updatePowerups(dt); updateParticles(dt); checkCollisions(); updateHud();
}
...
function draw() {
  ...
  stars.forEach(star => { ctx.fillStyle = `rgba(210,235,255,${.35 + star.size / 3})`; ctx.fillRect(star.x, star.y, star.size, star.size * 1.7); }); drawPowerups(); drawEnemies(); drawPlayer(); drawParticles(); ctx.restore();
...
function updateHud() { scoreValue.textContent = Math.floor(score); highScoreValue.textContent = highScore; livesValue.textContent = "❤️".repeat(lives) + "🖤".repeat(Math.max(0, 3 - lives)); cooldownHud.textContent = dodgeCooldown ? `Dodge: ${dodgeCooldown.toFixed(1)}s` : "Dodge: READY"; }
...
syncSoundButtons(); renderLeaderboard(); showScreen("homeScreen"); updateHud(); draw();
